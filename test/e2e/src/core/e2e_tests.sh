...
login_to_azure() {
	...
	if [[ -v SYSTEM_ACCESSTOKEN && -n "$SYSTEM_ACCESSTOKEN" &&
	  ...
	  az login --identity -u $WORKLOAD_CLIENT_ID 2> >(tee "${results_dir}/error" >&2) || python3 setup_failure_handler.py
	else
	  echo "logging in using service principal '${CLIENT_ID}'"
	  az login --service-principal \
	    -u ${CLIENT_ID} \
	    -p ${CLIENT_SECRET} \
	    --tenant ${TENANT_ID} 2> >(tee "${results_dir}/error" >&2) || python3 setup_failure_handler.py
	fi
