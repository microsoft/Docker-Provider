...
if !file.nil?
  ...
  file.write("export AZMON_COLLECT_STDOUT_LOGS=#{ConfigValue.to_shell_single_quoted(@collectStdoutLogs)}\n")
  file.write("export AZMON_LOG_TAIL_PATH=#{ConfigValue.to_shell_single_quoted(@logTailPath)}\n")
  logTailPathDir = File.dirname(@logTailPath)
  file.write("export AZMON_LOG_TAIL_PATH_DIR=#{ConfigValue.to_shell_single_quoted(logTailPathDir)}\n")
  file.write("export AZMON_LOG_EXCLUSION_REGEX_PATTERN=#{ConfigValue.to_shell_single_quoted(@logExclusionRegexPattern)}\n")
  file.write("export AZMON_STDOUT_EXCLUDED_NAMESPACES=#{ConfigValue.to_shell_single_quoted(@stdoutExcludeNamespaces)}\n")
  file.write("export AZMON_STDOUT_INCLUDED_SYSTEM_PODS=#{ConfigValue.to_shell_single_quoted(@stdoutIncludeSystemPods)}\n")
  file.write("export AZMON_COLLECT_STDERR_LOGS=#{ConfigValue.to_shell_single_quoted(@collectStderrLogs)}\n")
  file.write("export AZMON_STDERR_EXCLUDED_NAMESPACES=#{ConfigValue.to_shell_single_quoted(@stderrExcludeNamespaces)}\n")
  file.write("export AZMON_STDERR_INCLUDED_SYSTEM_PODS=#{ConfigValue.to_shell_single_quoted(@stderrIncludeSystemPods)}\n")
  file.write("export AZMON_CLUSTER_COLLECT_ENV_VAR=#{ConfigValue.to_shell_single_quoted(@collectClusterEnvVariables)}\n")
  file.write("export AZMON_CLUSTER_LOG_TAIL_EXCLUDE_PATH=#{ConfigValue.to_shell_single_quoted(@excludePath)}\n")
  file.write("export AZMON_CLUSTER_CONTAINER_LOG_ENRICH=#{ConfigValue.to_shell_single_quoted(@enrichContainerLogs)}\n")
  file.write("export AZMON_CLUSTER_COLLECT_ALL_KUBE_EVENTS=#{ConfigValue.to_shell_single_quoted(@collectAllKubeEvents)}\n")
  file.write("export AZMON_CONTAINER_LOGS_ROUTE=#{ConfigValue.to_shell_single_quoted(@containerLogsRoute)}\n")
  file.write("export AZMON_CONTAINER_LOG_SCHEMA_VERSION=#{ConfigValue.to_shell_single_quoted(@containerLogSchemaVersion)}\n")
  file.write("export AZMON_MULTILINE_ENABLED=#{ConfigValue.to_shell_single_quoted(@logEnableMultiline)}\n")
  file.write("export AZMON_MULTILINE_LANGUAGES=#{ConfigValue.to_shell_single_quoted(@stacktraceLanguages)}\n")
  file.write("export AZMON_KUBERNETES_METADATA_ENABLED=#{ConfigValue.to_shell_single_quoted(@logEnableKubernetesMetadata)}\n")
  file.write("export AZMON_KUBERNETES_METADATA_INCLUDES_FIELDS=#{ConfigValue.to_shell_single_quoted(@logKubernetesMetadataIncludeFields)}\n")
  file.write("export AZMON_ANNOTATION_BASED_LOG_FILTERING=#{ConfigValue.to_shell_single_quoted(@annotationBasedLogFiltering)}\n")
  if @isAzMonMultiTenancyLogCollectionEnabled
    file.write("export AZMON_MULTI_TENANCY_LOG_COLLECTION=#{ConfigValue.to_shell_single_quoted(@isAzMonMultiTenancyLogCollectionEnabled)}\n")
    file.write("export AZMON_MULTI_TENANCY_FALLBACK_INGESTION_DISABLED=#{ConfigValue.to_shell_single_quoted(@isAzMonMultiTenancyFallbackIngestionDisabled)}\n")
    file.write("export AZMON_MULTI_TENANCY_LOG_COLLECTION_ADVANCED_MODE=#{ConfigValue.to_shell_single_quoted(@isAzMonMultiTenancyLogCollectionAdvancedMode)}\n")
    azMonMultiTenantNamespacesString = @azMonMultiTenantNamespaces.join(",")
    file.write("export AZMON_MULTI_TENANCY_NAMESPACES=#{ConfigValue.to_shell_single_quoted(azMonMultiTenantNamespacesString)}\n")
    file.write("export AZMON_MULTI_TENANCY_STORAGE_MAX_CHUNKS_UP=#{ConfigValue.to_shell_single_quoted(@azMonMultiTenancyMaxStorageChunksUp)}\n")
    file.write("export AZMON_MULTI_TENANCY_SVC_BUFFER_CHUNK_SIZE=#{ConfigValue.to_shell_single_quoted(@azMonMultiTenancyServiceBufferChunkSize)}\n")
    file.write("export AZMON_MULTI_TENANCY_SVC_BUFFER_MAX_SIZE=#{ConfigValue.to_shell_single_quoted(@azMonMultiTenancyServiceBufferMaxSize)}\n")
  end
