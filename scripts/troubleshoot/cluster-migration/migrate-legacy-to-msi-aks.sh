...
#   - Supported clouds: Azure Public, Azure China, Azure US Government
#
# Usage:
...
if [[ "$(printf '%s\n' "2.49.0" "$current_version" | sort -V | head -n1)" != "2.49.0" ]]; then
    echo "[Error] Azure CLI version must be 2.49.0 or higher. Current version: $current_version"
    exit 1
fi
